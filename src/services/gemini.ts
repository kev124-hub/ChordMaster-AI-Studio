...
/**
 * Shared retry wrapper — handles 503 (service busy) and 429 (rate limited).
 */
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      if (
        ...
        throw new Error(
          "The AI returned an invalid response. Please try again or use a different song."
        );
      }
...
CONTENT: ${input}`;
    ...
    let content =
      input.type === "url"
        ? `Analyze this song from the URL: ${input.value}`
...
${frontendInstructions}`;
  ...
  const parsed = await withRetry(async () => {
    const result = await genAI.models.generateContent({
      ...
      config: {
        ...
        // Use HIGH thinking for uploaded files; MEDIUM for URL-based known tracks
        thinkingConfig: {
          thinkingLevel: isFile ? ThinkingLevel.HIGH : ThinkingLevel.MEDIUM,
        },
      },
    });

    const responseText = result.text || "{}";
    return JSON.parse(cleanJsonResponse(responseText)) as SongAnalysis;
  });
  ...
  parsed.fingerings = parsed.fingerings.map((f) => ({
    ...
    strings: Array.isArray(f.strings)
      ? f.strings
      : ["x", "x", "x", "x", "x", "x"],
  }));
