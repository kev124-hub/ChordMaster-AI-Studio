...
async function withRetry<T>(fn: () => Promise<T>, label: string): Promise<T> {
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      if ((isServiceBusy || isRateLimited) && attempts < maxAttempts) {
        console.log(`${label} error (attempt ${attempts}), retrying...`);
...
${frontendInstructions}`;
  ...
  const result = await withRetry(async () => {
    return await genAI.models.generateContent({
      ...
      config: {
        ...
        thinkingConfig: { thinkingLevel: isFile ? ThinkingLevel.HIGH : ThinkingLevel.MEDIUM },
      },
    });
  }, "analyzeSong");
  ...
  let parsed: SongAnalysis;
  try {
    parsed = JSON.parse(cleanJsonResponse(responseText)) as SongAnalysis;
  } catch {
